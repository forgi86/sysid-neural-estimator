...
if __name__ == "__main__":
    ...
    x_train = np.c_[q_train, v_train]
    t_fit, u_fit, x_fit = t_train[:n_fit], u_train[:n_fit], x_train[:n_fit]
    t_val, u_val, x_val = t_train[n_fit:] - t_train[n_fit], u_train[n_fit:], x_train[n_fit:]

    # %%  Prepare dataset, models, optimizer
    train_data = SubsequenceDataset(u_fit, x_fit, subseq_len=args.seq_len)
    train_loader = DataLoader(train_data, batch_size=args.batch_size, shuffle=True)

    val_data = SubsequenceDataset(torch.tensor(u_val), torch.tensor(x_val), subseq_len=args.seq_len)
    ...
    #f_xu = models.MechanicalTrigStateSpaceSystemV3(n_dof=n_dof, ts=ts, hidden_size=args.hidden_size, init_small=True).to(device)
    f_xu = models.MechanicalStateSpaceSystem(n_dof=n_dof, ts=ts, hidden_size=args.hidden_size,
    ...
    for epoch in tqdm.tqdm(range(args.epochs)):
        ...
        model.train()
        for batch_idx, (batch_u, batch_x) in enumerate(train_loader):
            ...
            batch_x = batch_x.transpose(0, 1).to(device)  # transpose to time_first
        ...
        with torch.no_grad():
            model.eval()
            for batch_u, batch_x in val_loader:
                batch_u = batch_u.transpose(0, 1).to(device)  # transpose to time_first
                batch_x = batch_x.transpose(0, 1).to(device)  # transpose to time_first
                batch_x0 = batch_x[0, :, :].squeeze(0)
    ...
    x_test = np.c_[q_test, v_test]
    with torch.no_grad():
        model.eval()
        #estimator.eval()
        u_v = torch.tensor(u_test[:, None, :]).to(device)
        y_v = torch.tensor(x_test[:, None, :]).to(device)
        # x0 = estimator(u_v, y_v)
        x0 = torch.zeros((1, n_x), dtype=u_v.dtype, device=u_v.device)
        x_sim = model(x0, u_v).squeeze(1).to("cpu").detach().numpy()
        q_sim = x_sim[:, :n_dof]
        v_sim = x_sim[:, n_dof:]
    # %% Metrics

    from torchid import metrics
    e_rms = metrics.rmse(x_test, x_sim)
    fit_idx = metrics.fit_index(x_test, x_sim)
    r_sq = metrics.r_squared(x_test, x_sim)
