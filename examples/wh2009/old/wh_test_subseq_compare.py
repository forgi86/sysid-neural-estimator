...
if __name__ == '__main__':
    ...
    model_data_1 = torch.load(os.path.join("models", "doe5", "model_389.pt"), map_location=torch.device('cpu'))  # worst Z
    model_data_2 = torch.load(os.path.join("models", "doe5", "model_5.pt"), map_location=torch.device('cpu')) # best
    #model_data = torch.load(os.path.join("models", "doe5", "model_578.pt"), map_location=torch.device('cpu'))  # worst

    batch_y_sim_full_all = []
    for model_idx, model_data in enumerate([model_data_1, model_data_2]):
        n_x = model_data["n_x"]
        n_y = model_data["n_y"]
        n_u = model_data["n_u"]
        ...
        with torch.no_grad():
            for batch_u, batch_y in loader:
                ...
                if args.est_type not in ["ZERO", "RAND"]:
                    batch_y_sim_full = torch.cat((torch.nan*torch.zeros((args.seq_est_len, args.batch_size, 1)),
                                                  batch_y_sim_full), 0)
        ...
        batch_y_sim_full_all.append(batch_y_sim_full_np)
